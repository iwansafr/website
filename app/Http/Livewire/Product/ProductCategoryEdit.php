...
class ProductCategoryEdit extends Component
{
    ...
    public function getParent()
    {
        ...
        if ($this->categoryId > 0) {
            $categoryParent = ProductCategory::where('parent',null)->whereNot('id',$this->categoryId)->get();
        }else{
            $categoryParent = ProductCategory::where('parent',null)->get();
        }
    ...
    public function render()
    {
        $this->getParent();
        $this->setSlug();
        return view('livewire.product.product-category-edit');
    }
    public function setSlug()
    {
        ...
        if (empty($this->categoryId)) {
            $slugCount = ProductCategory::where('slug', $slug)->count('id');
        }else{
            $slugCount = ProductCategory::where('slug', $slug)->whereNot('id', $this->categoryId)->count('id');
        }
        $numberSlug = '';
        if($slugCount > 0){
            $slugCount++;
